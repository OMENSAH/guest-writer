...
Also, you can observe that the `open` property has the value of `showPopOver` instead of the false value.  This `showPopOver` is obtained from the truthy value of `anchorEl`. Right after destructuring of the props;  `const { classes } = this.props; demonstrated below`, add the code below.
...
The `ListItem`  now has `onClick` property with `this.saveData` as a value. Hence you need to implement `saveData` as a method in the `MenuAppBar` component. Let's add that method.
...
When you click on `Add Event`, you can now see the alert dialog. For the user to store data, a form needs to be created so such user can input in his/her data before saving.  
