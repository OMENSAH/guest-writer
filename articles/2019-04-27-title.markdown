...
To get started with Material-UI, you have to add it as a dependency to your `React` project. You first create a `React` project with [create-react-app](https://facebook.github.io/create-react-app/), a tool that is developed and maintained by facebook team for creating pre-configured React project. 
...
Finally, you can test the project to make sure everything works fine. By issuing `npm start` command at the project's root directory in your terminal, the project will be served on development server via `http://localhost:3000/`. This must output empty content in the browser. Also, check the browser console to see if there is no error. 
...
Now that you have `Material-UI` added to your project, you have to use some of the components that come with.  For instance, you just add `Button` component from `Material-UI` to the project you created. In your `./src/App.js` file, 
...
This component will the root component that will hold other components. You go ahead to create our first component called the Navigation component.

### Developing the Navigation component 

Adding a navigation bar to the sample application can be implemented using `Appbar` component from the `Material-UI` library. To create this `Navigation` component, create , `components` sub-folder in `src` directory. In the `components` folder create `./NavBar/NavBar.js` file and add the code below to the newly created file.
...
There are other components like a drawer that you can add to this Navigation bar. To implement a drawer, import these extra dependencies that will be needed to create a basic drawer in the `NavBar` component.
...
With the drawer, you have to change the `NavBar` from functional component to class component so as to manage some states. The drawer needs to be toggled hence you got to manage its state of it being opened or closed.
...
You just added a Drawer with two list items to the navigation bar. However, you could see the drawer is always open. The point is to click on the  MenuIcon to toggle the drawer. Adding up that functionality of opening or closing a drawer to your `NavBar` component, then you should update `NavBar` class to the code below.
...
In all, your `NavBar` component's code must be like this;
...
```
Few styles have been added to improve the Link. After saving the file and testing in the browser, you will get some errors. This is basically telling us that the whole app is not set up to have routes. To solve this, you wrap the whole application to use routes. Go to `./src/index.js` and update the contents to;
...
The `ListItem`  now has `onClick` property with `this.handleClickOpenDialog` as a value. Hence you need to implement `handleClickOpenDialog` as a method in the `NavBar` component. You add that method.
...
Now you can see in your browser that the `Dialog` box is automatically shown. This is because `Dialog` component has `open` propert which is currently set to `true` and `onClose` to `false`. This means the state of the `Dialog` must be manipulated to be opened and closed on certain conditions.  Since you are using the `AddEvent` component in `NavBar` component, you can manipulate the state of the  `Dialog` component in `NavBar` component and then pass the result to the `AddEvent` component via props. In the `NavBar` component, add `openDialog` as key to the state object with a value of `false` as demonstrated below. 

```js
openDialog: false
```

Update `handleClickOpenDialog` to show the dialog by setting the value of `openDialog` to `true`.  Also, add another method named `handleCloseDialog` in the `NavBar` component to reset the of the `openDialog`  value to `false`.
...
You can now pass the `openDialog` state object key to the `AddEvent` component as discussed earlier. In the `NavBar` component, add the code below as additional `prop` to `AddEvent` 
...
`Snackbar` component presents a single line of text directly related to the operation performed. You can use it to display notifications. Implement `Snackbar` component by creating a sub-folder in the `component` named `Snackbar` with `Snackbar.js` file. In the `Snackbar.js` file, add the following lines of code.
...
This means the state of snackbar must be controlled. Add extra state property,`showSnackbar`, with a value of `false` to the state object in the `AddEvent` component. You update the `saveData` method to set `showSnackbar` to `true`;
...
You can equally create a method to do so. Add a method `handleDialogCancel` to the `AddEvent` component with the code below;
...
But there is an issue, same message, and color of dialog box. Customize `MySnackbarContentWrapper` to have a custom message and a color of the dialog box depending on the state of the form whether it was canceled or successfully submitted. This means you have to pass props around the `MySnackbarContentWrapper` component. The main props that you will need are `variant` and `message` for the dialog color and the content respectively.
...
The NavBar houses `Add Event`, `Logout` and `My Account` labels. The `Add Event` label shows the addEvent dialog resides in the NavBar component.  That item should only be shown when the user is logged in else show nothing. You got to update that portion of code as well. Before that, you need to pass the auth instance to the NavBar component through probs. Therefore you have to update the `<NavBar createEvent={this.createEvent}/>` in the App.js to `<NavBar createEvent={this.createEvent} auth={this.auth}/>`
