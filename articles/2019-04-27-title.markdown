...
Here, you are using `IconButton` to create popup menu elements. This menu has two elements, `My Account` and `Logout` where `My Account` is a link to `/account` url. In the menu, there is an `open` props that is currently set to false. Once it is true, the pop will show. This means we have to handle its state as well.
...
To make the cells of the table to have a common view, a Higher Order Component(HOC), `CustomTableCell` was created to make use of the customizations needed for the cell. Customizations like `background` color for the table head, `font-size` for the table body have been added to this HOC. 
...
When the data is successfully saved, you then see a alert box. Let's replace this with `Material-UI`'s snackbar component.

### Adding Snackbar Component.

`Snackbar` component presents a single line of text directly related to the operation performed. They may contain a text action, but no icons. You can use them to display notifications. Let's implement this by creating a subfolder in the `component` named `Snackbar` with `Snackbar.js` file. In the `Snackbar.js` file, add the following lines of code.
...
The `MySnackbarContent` component  basically has `message`that it will be displayed and `action` to close the snackbar. Currently, youu have a prepopulated message and `IconButton` that handle the action of the snackbar. Let's go ahead to use this. 

In the `AddEvent` component, you need to include the snackbar and that can be achieved by wrapping the ` MySnackbarContent` in `Snackbar` component. For the The root element of `AddEvent` component is `Dialog`, because you need to add extra component let' wrap the `Dialog` and `Snackbar` components in `div` element as shown below.
...
The snackbar is showing but it is supposed to be shown after successful submission of the form. Hence the state of the snacbar must be controlled. Initially, it should not be shown and later get shown, so it is just a manipualation of boolean value. Add extra `showSnackbar` with a value of false to the state object in the `AddEvent` component. Let's update the `saveData` method to manipulate this `showSnackbar` state value.
...
What if you want you want to show a snackbar after closing the dialog form. You can achieve that as well. In the form dialog, there are two buttons, `CANCEL` and `SEND`. To close the dialog, you can have just used the ` this.props.onCloseDialog()` on `onClick` props but in this case you have extra functionality to be accomplished by the `CLOSE` button. You can equally create a method to do so. Let's add a method `handleDialogCancel` to the `AddEvent` component with the code below;
...
Here, you a manipulating a state called `isCancelled` to a truthy value. Hence you will need to `isCancelled` to the state object with a value of `false`. 
To make this method to be executed, you have to pass it as a reference to an `onClick` prop of the `CANCEL` button as ;
...
But there is an issue, same message and color of dialog box. Let's customize `MySnackbarContentWrapper` to have a custome message and a color of the dialog box that depicts it. For instance, we can make the dialog looks successfully when data is saved  or erronous when form gets cancelled. That means we have to pass props around the `MySnackbarContentWrapper`. The main props that you will need are variant and message for the dialog color and the content respectively.
...
```
**NB** Sometimes, the `Snackbar` component autoHideDuration does not work, so you can manually close with onClose props and then pass a the method reference that hides the snackbar as shown. For instance, this method will hide the snackbar when passed as reference to Snackbar's `onClose` prop.
