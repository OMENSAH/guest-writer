...
Finally, building React applications means you will be working with Command Line Interface(CLI) a lot. Hence you need to access to your terminal or command prompt.

## What You Will Build

You will be building a sample application that utilizes most of the concept you will be learning from the walk-through of Material-UI. This sample project allows users authenticated with Auth0 to create events as well as view their Auth0 profile details. 
...
Use the `create-react-app` command to create your React project by issuing the command below;
...
Creating React project with `create-react-app` command will generate a pre-configured React project with certain files and folders. Depending on your project scope, some of the files will not be needed hence you can get ride of them. To clean up the project, you will get rid of `App.test.js`, `logo.svg`, `serviceworkers.js` files in `./src` folder. Also, remove the content of `App.css`, `index.css` and update `index.js` to have the content below.
...
Finally, you can test the project to make sure everything works fine by issuing `npm start` command at the project's root directory in your terminal.  This will serve the project will be on development server via http://localhost:3000/. You will see an empty page as there is nothing yet to be displayed. Also, check the browser console to see if there is no error. 
...
By testing few components from Material-UI,  update `./src/App.js` content as follows;
...
Most of these props will be covered later. However, you can explore the properties of each component by visiting the Material-UI Component APIs documentation.  For instance the Button Component API has its documentation available at [https://material-ui.com/api/button/](https://material-ui.com/api/button/). To see other component's API, go through Component API drawer item of [the Material-UI Website](https://material-ui.com).


### Installing and Working with Icons

React Material-UI provides pre-built icons that you can use in your project. They don't come with the Material-UI package so you need to install the package for the icons as well. Go ahead to install the icons package with this command;

```bash
npm install @material-ui/icons
```
Testing the icons as well, update the `./src/App.js` file as follows;
...
Here, you just imported Info icon to the project. 
...
Having worked with few components from the React Material-UI library, you will then start to work on the sample project. Visiting the sample project, the user interface consists of a lot of material design elements. You will make use of Material-UI components to display the tabular list of events' data as well create a form to create an event. It also consists of the navigation bar which contains navigation drawer, dropdown list as well as navigation links. 

In terms of structure, the project will have components folder to hold components that can be used in different components. It will also contain pages folder to hold all components that would be used as pages. And a service folder to hold third party services like Auth0 that would be used in the application. Finally, `./src/App.js` will eb the root component to load other components and pages of the entire application. 

To begin implementing the sample project, you will need to clean up the `App.js` to:
...
Adding a navigation bar to the sample application can be implemented using AppBar component from the Material-UI library.  Create a `components` sub-folder in `src` directory. In the `components` folder, create `./NavBar/NavBar.js` file and add the code below to the newly created file.
...
Here, you are making use of AppBar, Toolbar and Typography components from the Material-UI library to create the navigation bar. The component is then exported.  The sample project has a common navigation bar hence you need to import and use it at the root component, `./src/App.js` as follows;
...
You got your first component implemented, you can go ahead to test it your browser.  There are other components like a drawer that you can add to this Navigation bar. To implement a drawer, import these extra dependencies that will be needed to create a basic drawer in the NavBar component.
...
You just added a Drawer with two list items to the navigation bar. However, you could see the drawer is always open. Ideally, the MenuIcon is used to toggle the drawer. Adding up that functionality of opening or closing a drawer to your NavBar component, then you should update `NavBar.js` file to the code below.
...
Here, you added openDrawer property with a default value of false to the state object of the `NavBar` component. This state property will keep track whether the drawer is opened or closed. The openDrawer state property is then manipulated by two methods; `handleDrawerOpen` and `handleDrawerClose` where it is reset to true and false respectively. When the MenuIcon is clicked, `handleDrawerOpen` method is called and thus changing the state of the drawer to true.  The drawer has ChevronLeftIcon (<)  which when clicked closes the drawer by also referencing `handleDrawerClose` method.  

You might have noticed already that when the MenuIcon is clicked to open the drawer, the drawer covers the MenuIcon and the text on the navigation. You can shift the navigation to the right when the MenuIcon is clicked and when it is closed, it should get back to its original position. This is achieved by just adding some few css styles to the AppBar using [CSS-in-JS](https://material-ui.com/customization/css-in-js/) approach. This approach uses higher-order component, `withStyles`, to inject an array of styles into the DOM as CSS using JS.
...
To inject that styles you created to have an effect on the component you have to export `NavBar` as module wrapped with a higher order function as shown below;
...
Now, the navigation should look quite better after clicking the MenuIcon. But you can see there is a lot of space between the Drawer and the AppBar. This can be solved by adding a few CSS style.  In the CSS you created, add the following pieces of code;
...
From the above CSS styles, menuButton will help create a margin between the menuIcon and the text. drawer provides the width of the drawer while  drawerPaper and drawerHeader removes the margin between the drawer and the toolbar.

Add these CSS styles to the few components like IconButton.
...
You are almost done implementing the navigation bar but so far you have not added any navigation links yet.  You have a Home text on the navigation bar, you can convert it to navigation link. 

You will be using `react-router-fom`  package to handle the navigation since React does not come with routing implementation. Use the command below to do the installation.
...
Here, rew styles have been added to remove underline and make the text white. After saving the file and testing in the browser, you will get some errors. This is basically telling us that the whole app is not set up to have routes. To solve this, you wrap the whole application to use routes. Go to `./src/index.js` and update as follows;
...
Now when you hover over the home item on the navigation bar it turns into a clickable link.  Create another link item on the navigation bar after Home text as;
...
Then update the parent div of the component and the second navigation link item to have the styles you just added;
...
Also, in our drawer, the About item will be a link to `/about` page hence you need to implement navigation link for that as well.  Update that item as shown below;
...
Lastly, there is one feature on the navigation bar which has not yet been implemented. and that is the dropdown menu which has two items, My Account  and Logout. To implement these, you need to import some components and icons from the Material-UI and the icons  libraries. 
...
From here, you can add these lines of code to the Toolbar. In the toolbar, right after the second navigation link, Dashboard, add these codes.
...
Here, IconButton will be used to create popup menu elements. This menu has two elements, My Account and Logout, where My Account is a link to `/account` page. 
...
The homepage has just a description of the application as well as a button for the user to login. You go ahead to implement the Homepage component. 

In your root directory, create a new folder called `pages`. Create a subdirectory called `HomePage` which will hold `Homepage.js` file. In the newly created file., you add these content;
...
Here, you are using Button and Typography components from Material-UI. Also, there an external CSS file, `HomePage.css` that is imported. Create such file in the `HomePage` folder. After creating such file, add these CSS codes to it.
...
In `App.js` file, import Routes from `react-router-dom` and also import HomePage and About components  as shown below;
...
Now, you click on the About link, you can hide the drawer. You just have to implement hiding functionality. To do so go to the `NavBar.js` file, then add the code below as a property of the NavLink that wraps the About item.  
...
The Dashboard page will hold a tabular representation of the stored data. Hence, it will mostly use components that related to the Table module. You can start importing the modules from the Material-UI that you will need to tabular view. You do that together by creating a `./Dashboard/Dashboard.js` file in the `pages` subfolder.
...
The Dashboard implementation is similar to other components that you have implemented before just that you are using different components. Generally, you have a reference to CSS classes for styling the page, importing components, creating the contents and exporting the component mostly wrapped in  `withStyles` higher order component for the CSS styling to take effect. The Dashboard component is also dealing with dynamic rendering where a tabular view is created after data is stored from the form dialog. If there is no data a different view with a custom data is displayed.
...
For the table to populate content, there must a functionality to store data into the application. If you remember, the application had label, Add Event,  and it tells an action to record an event in application. However, on clicking this label, a form dialog needs to be shown to collect the data needed to create the Dashboard table.  
...
When you click on Add Event label, you can now see the alert dialog. For the user to store data, a form needs to be created so such user can input in his/her data before saving. In your `components` folder, create a subfolder named `AddEvent` with `AddEvent.js` file. Then add the code below to this file.
...
Here,, you are creating a form with Dialog module.  The Dialog module presents DialogActions where actions can be perform on the dialog can be positioned. It also contains DialogContent section to hold the actual content, in this case, it is a form, There is also DialogTitle for its caption. 
...
Defining the data-store for the captured data in `App.js` will be very much appropriate. Since `AddEvent` component is nested in `NavBar` component. Hence the data-store must be accessible by both `NavBar` and `Dashboard` components so that it can be passed around to where it is needed very most. 
...
```
This method accepts the event data and callback method that would be executed after you have successfully saved the data to the dataStore. To make this method executed in the `AddEvent` component, you have to pass it through `NavBar` using props as shown below'

```js
<NavBar createEvent={this.createEvent} />
```

In the `NavBar` component, you are passing the receiving props like a property to `AddEvent`;
...
What if you want to also show a snackbar after closing the dialog form? In the form dialog, there are two buttons, CANCEL and SEND. To close the dialog, you could have executed `this.props.onCloseDialog` reference on onClick property but in this case, you have extra functionality to be accomplished by the CLOSE button. 
